...
| `promote-prod.yml` | `promote.yml` behind the change-record gate and the production environment | + `jira-issue-key`, `jira-digest-field` (set it — otherwise only ticket *status* is checked, not ticket↔digest equality) | |
